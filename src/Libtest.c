...
int main(void)
{
	 char stringMatrix[4][256] =
	 { "3x + 4y -5z + 6a = 39", "6x + 5y - 6z + 5a = 43",
	 "9x - 4y + 2z + 3a = 6", "2y - 3z + a = 13" };

	double matrixr[4];

	char *sMatrix[4];
	for (int i = 0; i < 4; i++)
		sMatrix[i] = stringMatrix[i];

	static double srMatrix[4][5];
	double *srrMatrix[4];
	for (int i = 0; i < 4; i++)
		srrMatrix[i] = srMatrix[i];

	seEvaluate(sMatrix, 4, srrMatrix);
	lsoeSolve(srrMatrix, matrixr, 4);
